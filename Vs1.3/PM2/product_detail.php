...
<script>
    ...
    function submitInstallment(e) {
        ...
        .then(data => {
            if (data.success) {
                alert('Đăng ký tư vấn trả góp thành công! Hệ thống đã ghi nhận yêu cầu của bạn.');
                document.getElementById('installmentModal').classList.add('hidden'); // Đóng form
                document.getElementById('installmentForm').reset(); // Xóa form
            } else {
    ...
    // Hàm xử lý Thêm vào giỏ bằng AJAX
    function addToCartAjax(productId) {
        ...
        .then(text => {
            try {
                ...
                alert('Đã thêm vào giỏ hàng! (Hãy tải lại trang để xem số lượng)');
            }
        })
        .catch(error => {
