...
// Chặn Quản lý (Manager) truy cập vào trang Users
if ($user_role === 'manager' && $page === 'users') {
    $page = 'products';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_product' || $action === 'edit_product') {
        ...
        // 1. XỬ LÝ TẢI ẢNH (UPLOAD FILE)
        $image = $_POST['image']; // Mặc định lấy từ ô URL nếu có
        
        // Nếu người dùng có chọn file từ máy tính
    ...
    if ($user_role === 'admin') {
        if ($action === 'add_user' || $action === 'edit_user') {
            ...
            $id = $_POST['id'];
            if ($id != $_SESSION['user_id']) { // Không tự xóa mình
...
// Dữ liệu Sản phẩm
if ($page === 'products') {
    $sql = "SELECT p.*, c.name as cat_name, b.name as brand_name FROM products p JOIN categories c ON p.category_id=c.id JOIN brands b ON p.brand_id=b.id";
...
<head>
    ...
    <!-- Tích hợp TinyMCE (Trình soạn thảo văn bản như Word) -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/tinymce/6.8.2/tinymce.min.js"></script>
    <script>
        // Cấu hình TinyMCE cho 2 thẻ textarea
        tinymce.init({
...
<body class="bg-gray-100 font-sans flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 text-white flex flex-col h-full shadow-2xl z-20">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            <div class="text-xs text-slate-400 font-bold mb-4 uppercase tracking-wider">Quản lý Dữ liệu</div>
            ...
            <a href="?p=users" class="flex items-center gap-3 px-4 py-3 rounded-lg transition <?= $page==='users' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800' ?>">
                <i class="fa-solid fa-users w-5"></i> Tài khoản (Admin)
            </a>
            <?php endif; ?>
        </nav>
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 z-10 shrink-0">
            <h2 class="text-xl font-bold text-gray-800">
                <?= $page === 'products' ? 'Quản lý Sản Phẩm' : 'Quản lý Tài Khoản' ?>
            </h2>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-50 border-b border-gray-100 transition">
                                ...
                                <td class="p-4">
                                    <div class="text-gray-800 font-medium"><?= $item['brand_name'] ?></div>
                                    <div class="text-gray-400 text-xs"><?= $item['cat_name'] ?></div>
                                </td>
    ...
    <div id="productModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative">
            ...
            <!-- ĐÃ THÊM enctype="multipart/form-data" ĐỂ HỖ TRỢ UPLOAD FILE -->
            <form method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Hoặc dùng URL Ảnh (Bỏ trống nếu tải ảnh)</label>
                ...
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                    ...
                    <!-- Thêm onclick triggerSave để TinyMCE đồng bộ dữ liệu trước khi gửi form -->
    ...
    <!-- SCRIPTS JS -->
    <script>
        // Hiển thị thông báo (Alert) bằng SweetAlert2
        <?php if($msg): ?>
            Swal.fire({
                icon: '<?= $msg_type ?>',
        ...
        // Hàm Xóa (Cảnh báo trước khi xóa)
