...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_order_id'])) {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($order_to_cancel && $order_to_cancel['status'] === 'pending') {
            ...
            $cancel_error = 'Chỉ được phép hủy đơn hàng ở trạng thái "Chờ xử lý" (COD).';
        }
    } else {
        $cancel_error = 'Bạn cần đăng nhập để thực hiện thao tác này.';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!isset($_SESSION['user_id'])) {
        $action_msg = 'Bạn cần đăng nhập để thao tác.';
    } else {
        ...
        if ($action === 'request_warranty') {
            ...
            if (empty($reason)) {
                $action_msg = 'Vui lòng nhập lý do bảo hành.';
            } else {
                ...
                $action_success = true;
                $action_msg = 'Đã gửi yêu cầu bảo hành thành công. Chúng tôi sẽ liên hệ trong 24h.';
            }
            ...
            if (empty($reason)) {
                $action_msg = 'Vui lòng nhập lý do trả hàng / hoàn tiền.';
            } else {
                ...
                $action_success = true;
                $action_msg = 'Đã gửi yêu cầu trả hàng / hoàn tiền. Vui lòng chờ bộ phận CSKH liên hệ.';
            }
        }
    }
}
...
if ($search_query !== '') {
    ...
    if (empty($orders)) {
        $error = 'Không tìm thấy đơn hàng nào khớp với thông tin: <b>' . htmlspecialchars($search_query) . '</b>';
    }
}
/**
...
<div class="container mx-auto px-4 py-10 max-w-5xl min-h-[60vh]">
        ...
        <div class="bg-white border-b border-gray-200 mb-6 sticky top-[60px] z-40 shadow-sm rounded-t-xl overflow-hidden">
            <div class="flex overflow-x-auto hide-scrollbar text-[14px] font-medium">
                ...
                <a href="?status=pending"
                    ...
                    Chờ xử lý (<?= $status_counts['pending'] ?? 0 ?>)
                </a>
                <a href="?status=processing"
                    ...
                    Đã thanh toán (<?= $status_counts['processing'] ?? 0 ?>)
                </a>
                <a href="?status=delivering"
                    ...
                    Đang giao (<?= $status_counts['delivering'] ?? 0 ?>)
                </a>
                <a href="?status=completed"
                    ...
                    Hoàn thành (<?= $status_counts['completed'] ?? 0 ?>)
                </a>
                <a href="?status=cancelled"
                    ...
                    Đã hủy (<?= $status_counts['cancelled'] ?? 0 ?>)
                </a>
                ...
                <a href="?status=warranties"
                    ...
                    <i class="fa-solid fa-wrench text-[11px] mr-1"></i>Bảo hành (<?= count($user_warranties ?? []) ?>)
                </a>
                ...
                <a href="?status=returns"
                    ...
                    <i class="fa-solid fa-right-left text-[11px] mr-1"></i>Đổi trả (<?= count($user_returns ?? []) ?>)
                </a>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if ($status_filter === 'warranties' && isset($_SESSION['user_id'])): ?>
        <?php if (empty($user_warranties)): ?>
            <div class="bg-white p-10 rounded-xl border border-gray-200 text-center shadow-sm">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">Chưa có yêu cầu bảo hành nào</h3>
                <p class="text-gray-500 mb-6">Bạn chưa gửi yêu cầu bảo hành sản phẩm nào.</p>
            </div>
        <?php else: ?>
            <div class="space-y-5">
                <?php foreach ($user_warranties as $w):
                    ...
                    $w_steps = [
                        ['label' => 'Tiếp nhận yêu cầu', 'icon' => 'fa-inbox', 'done' => true],
                        ['label' => 'Đang kiểm tra & xử lý', 'icon' => 'fa-gear', 'done' => in_array($w['status'], ['processing', 'completed', 'rejected'])],
                        ['label' => $w['status'] === 'rejected' ? 'Từ chối bảo hành' : 'Hoàn thành bảo hành', 'icon' => $w['status'] === 'rejected' ? 'fa-xmark' : 'fa-circle-check', 'done' => in_array($w['status'], ['completed', 'rejected'])]
                    ];
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                    ...
                    <div class="bg-gray-50 p-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500">Đơn hàng #<?= $w['order_id'] ?> · Yêu cầu lúc <?= date('H:i d/m/Y', strtotime($w['created_at'])) ?></div>
                            </div>
                        </div>
                    ...
                    <div class="p-5">
                        ...
                        <div class="mb-5 bg-gray-50 rounded-lg p-3 border border-gray-100">
                            <div class="text-xs font-bold text-gray-500 mb-1"><i class="fa-solid fa-quote-left mr-1"></i>Lý do bảo hành:</div>
                        ...
                        if (!empty($w_media)): ?>
                        <div class="mb-5">
                            <div class="text-xs font-bold text-gray-500 mb-2"><i class="fa-solid fa-images mr-1"></i>Bằng chứng đính kèm:</div>
                        ...
                        <div class="mt-5 bg-blue-50 border border-blue-200 rounded-xl p-4 relative">
                            <div class="absolute -top-3 left-4 bg-blue-500 text-white text-[10px] font-bold px-3 py-0.5 rounded-full flex items-center gap-1">
                                <i class="fa-solid fa-headset"></i> Phản hồi từ cửa hàng
                            </div>
    ...
    <?php elseif ($status_filter === 'returns' && isset($_SESSION['user_id'])): ?>
        <?php if (empty($user_returns)): ?>
            <div class="bg-white p-10 rounded-xl border border-gray-200 text-center shadow-sm">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">Chưa có yêu cầu đổi trả nào</h3>
                <p class="text-gray-500 mb-6">Bạn chưa gửi yêu cầu trả hàng / hoàn tiền nào.</p>
            </div>
        <?php else: ?>
            <div class="space-y-5">
                <?php foreach ($user_returns as $r):
                    ...
                    $r_steps = [
                        ['label' => 'Đã tiếp nhận yêu cầu', 'icon' => 'fa-inbox', 'done' => true],
                        ['label' => 'Đang lấy hàng hoàn', 'icon' => 'fa-truck-ramp-box', 'done' => in_array($r['status'], ['approved', 'refunded', 'rejected'])],
                        ['label' => 'Đang kiểm tra lỗi', 'icon' => 'fa-magnifying-glass', 'done' => in_array($r['status'], ['approved', 'refunded'])],
                        ['label' => $r_is_rejected ? 'Từ chối yêu cầu' : 'Đã hoàn tiền', 'icon' => $r_is_rejected ? 'fa-xmark' : 'fa-money-bill-wave', 'done' => in_array($r['status'], ['refunded', 'rejected'])]
                    ];
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                    ...
                    <div class="bg-gray-50 p-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                        <div>
                            <div class="font-bold text-gray-800 text-sm"><i class="fa-solid fa-right-left text-purple-500 mr-1"></i>Yêu cầu trả hàng — Đơn #<?= $r['order_id'] ?></div>
                            <div class="text-xs text-gray-500">Gửi lúc <?= date('H:i d/m/Y', strtotime($r['created_at'])) ?></div>
                        </div>
                    ...
                    <div class="p-5">
                        <!-- Lý do trả hàng -->
                        <div class="mb-5 bg-gray-50 rounded-lg p-3 border border-gray-100">
                            <div class="text-xs font-bold text-gray-500 mb-1"><i class="fa-solid fa-quote-left mr-1"></i>Lý do trả hàng:</div>
    ...
    <?php elseif ($search_query !== '' || isset($_SESSION['user_id'])): ?>
        ...
        <?php if (empty($orders)): ?>
            <div class="bg-white p-10 rounded-xl border border-gray-200 text-center shadow-sm">
                ...
                <p class="text-gray-500 mb-6"><?= $error ? $error : 'Bạn không có đơn hàng nào ở trạng thái này.' ?></p>
                <a href="index.php"
        ...
        <?php else: ?>
            <?php if ($search_query !== ''): ?>
                <h3 class="text-lg font-bold text-gray-800 mb-4 border-b pb-2">
                    Kết quả tìm kiếm: Đã tìm thấy <?= count($orders) ?> đơn hàng
                </h3>
            <?php endif; ?>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden transition hover:shadow-md">
                        ...
                        <div class="p-4 md:p-5 grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="md:col-span-1 space-y-3 text-sm">
                                <h4 class="font-bold text-gray-800 border-b border-gray-100 pb-2"><i
                                        class="fa-solid fa-address-card text-primary mr-1"></i> Thông tin nhận hàng</h4>
                                <p><span class="text-gray-500 inline-block w-20">Họ tên:</span> <b
                                        class="text-gray-800"><?= htmlspecialchars($order['fullname']) ?></b></p>
                                <p><span class="text-gray-500 inline-block w-20">Điện thoại:</span> <b
                                        class="text-gray-800"><?= htmlspecialchars($order['phone']) ?></b></p>
                                <p class="flex"><span class="text-gray-500 inline-block w-20 shrink-0">Địa chỉ:</span> <span
                                        class="text-gray-800 leading-snug"><?= htmlspecialchars($order['address']) ?></span></p>
                                <?php if ($order['note']): ?>
                                    <p class="flex"><span class="text-gray-500 inline-block w-20 shrink-0">Ghi chú:</span> <span
                            ...
                            <div class="md:col-span-2">
                                <h4 class="font-bold text-gray-800 border-b border-gray-100 pb-2 mb-3"><i
                                        class="fa-solid fa-box-open text-primary mr-1"></i> Sản phẩm đã đặt</h4>
                                                ...
                                                <div class="flex justify-between items-center text-sm">
                                                    ...
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        <?php if ($order['status'] === 'completed'): ?>
                                                            <button type="button" onclick="openWarrantyModal(<?= $order['id'] ?>, <?= $item['product_id'] ?>)" class="text-[11px] bg-white border border-gray-300 text-gray-600 px-2 py-0.5 rounded hover:bg-gray-50 hover:text-blue-600 transition" title="Yêu cầu bảo hành SP này"><i class="fa-solid fa-wrench"></i> Bảo hành</button>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <?php if ($order['status'] === 'pending'): ?>
                            <div
                                ...
                                <span class="text-[13px] text-blue-800"><i class="fa-solid fa-circle-info mr-1"></i> Đơn hàng đang chờ
                                    xử lý. Bạn có thể thanh toán QR hoặc hủy đơn.</span>
                                <div class="flex items-center gap-2 shrink-0">
                                    <a href="payment.php?order_id=<?= $order['id'] ?>"
                                        class="text-[12px] bg-primary text-white px-4 py-1.5 rounded font-bold hover:bg-blue-800 transition shadow-sm whitespace-nowrap">Thanh
                                        toán QR</a>
                                    <button type="button" onclick="openCancelModal(<?= $order['id'] ?>)"
                                        class="text-[12px] bg-white text-red-600 border border-red-300 px-4 py-1.5 rounded font-bold hover:bg-red-50 transition shadow-sm whitespace-nowrap"><i
                                            class="fa-solid fa-xmark mr-1"></i>Hủy đơn</button>
                                </div>
                            </div>
                        ...
                        <?php elseif ($order['status'] === 'cancelled'): ?>
                            <div class="bg-gray-50 px-5 py-3 border-t border-gray-200 text-center">
                                <span class="text-[13px] text-gray-500"><i class="fa-solid fa-clock-rotate-left mr-1"></i> Bạn có thể
                                    đặt lại sản phẩm này bất kỳ lúc nào tại danh mục sản phẩm.</span>
                            </div>
                        ...
                        <?php elseif ($order['status'] === 'completed'): ?>
                            <div class="bg-green-50 px-5 py-3 border-t border-green-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                <span class="text-[13px] text-green-800"><i class="fa-solid fa-box-check mr-1"></i> Đơn hàng đã giao thành công. Bạn có thể yêu cầu trả hàng nếu sản phẩm lỗi.</span>
                                <div class="flex items-center gap-2 shrink-0">
                                    <button type="button" onclick="openReturnModal(<?= $order['id'] ?>)" class="text-[12px] bg-white text-gray-700 border border-gray-300 px-4 py-1.5 rounded font-bold hover:bg-gray-100 transition shadow-sm whitespace-nowrap"><i class="fa-solid fa-right-left mr-1"></i>Trả hàng / Hoàn tiền</button>
                                </div>
                            </div>
                        <?php endif; ?>
    ...
    <?php if (isset($cancel_success) && $cancel_success): ?>
        <div id="cancelSuccessToast"
            ...
            <span class="font-medium">Đơn hàng đã được hủy thành công!</span>
        </div>
    <?php endif; ?>
...
<div id="cancelModal"
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 text-center transform scale-95 opacity-0 transition-all duration-300"
        ...
        <h3 class="text-xl font-bold text-gray-800 mb-2">Xác nhận hủy đơn hàng?</h3>
        <p class="text-gray-600 mb-1">Bạn đang yêu cầu hủy đơn hàng:</p>
        ...
        <p class="text-sm text-gray-500 mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            ...
            Hành động này <b>không thể hoàn tác</b>. Đơn hàng sau khi hủy sẽ không thể khôi phục.
        </p>
        <div class="flex gap-3 justify-center">
            <button onclick="closeCancelModal()"
                class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-bold hover:bg-gray-200 transition border border-gray-200 shadow-sm">Không,
                giữ lại</button>
            <form method="POST"
                ...
                <button type="submit"
                    class="px-6 py-2.5 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700 transition shadow-md"><i
                        class="fa-solid fa-ban mr-1"></i> Xác nhận hủy</button>
            </form>
        </div>
    </div>
</div>
...
<div id="warrantyModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform scale-95 opacity-0 transition-all duration-300 relative" id="warrantyModalContent">
        ...
        <h3 class="text-xl font-bold text-gray-800 mb-2">Yêu Cầu Bảo Hành</h3>
        <p class="text-gray-500 text-sm mb-4">Vui lòng mô tả lỗi và đính kèm ảnh/video bằng chứng để xử lý nhanh hơn.</p>
        ...
        <form method="POST" action="track_order.php<?= isset($_GET['status']) ? '?status=' . htmlspecialchars($_GET['status']) : '' ?>" enctype="multipart/form-data">
            ...
            <div class="mb-4 text-left">
                <label class="block text-sm font-bold text-gray-700 mb-1">Mô tả lỗi gặp phải <span class="text-red-500">*</span></label>
                <textarea name="reason" rows="3" class="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ví dụ: Cắm điện không lên nguồn, màn hình bị sọc..." required></textarea>
            </div>
