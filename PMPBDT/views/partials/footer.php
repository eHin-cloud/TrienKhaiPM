    ...
    <div class="container mx-auto px-4 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            ...
            <div>
                ...
                <ul class="space-y-4">
                    <li class="flex items-start gap-3 group">
                        ...
                        <span class="text-sm text-gray-300 leading-snug">53 Võ Văn Ngân, Phường Thủ Đức, TP. Hồ Chí Minh</span>
                    </li>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a href="javascript:void(0)" onclick="showPolicyModal('warranty')" class="text-sm text-gray-300 hover:text-secondary transition-colors flex items-center gap-2 group"><i class="fa-solid fa-chevron-right text-[10px] text-secondary group-hover:translate-x-1 transition-transform"></i> <?= __("warranty_policy") ?></a></li>
                    <li><a href="javascript:void(0)" onclick="showPolicyModal('return')" class="text-sm text-gray-300 hover:text-secondary transition-colors flex items-center gap-2 group"><i class="fa-solid fa-chevron-right text-[10px] text-secondary group-hover:translate-x-1 transition-transform"></i> <?= __("return_policy_1_1") ?></a></li>
                    <li><a href="javascript:void(0)" onclick="showPolicyModal('delivery')" class="text-sm text-gray-300 hover:text-secondary transition-colors flex items-center gap-2 group"><i class="fa-solid fa-chevron-right text-[10px] text-secondary group-hover:translate-x-1 transition-transform"></i> <?= __("delivery_install") ?></a></li>
                    <li><a href="javascript:void(0)" onclick="showPolicyModal('installment')" class="text-sm text-gray-300 hover:text-secondary transition-colors flex items-center gap-2 group"><i class="fa-solid fa-chevron-right text-[10px] text-secondary group-hover:translate-x-1 transition-transform"></i> <?= __("installment_guide") ?></a></li>
                    <li><a href="javascript:void(0)" onclick="showPolicyModal('faq')" class="text-sm text-gray-300 hover:text-secondary transition-colors flex items-center gap-2 group"><i class="fa-solid fa-chevron-right text-[10px] text-secondary group-hover:translate-x-1 transition-transform"></i> <?= __("faq") ?></a></li>
                </ul>
            </div>
        ...
        <div class="pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex gap-4 text-xs text-blue-100">
                <a href="javascript:void(0)" onclick="showPolicyModal('terms')" class="hover:text-white transition"><?= __("terms_of_use") ?></a>
                <span class="text-white/20">|</span>
                <a href="javascript:void(0)" onclick="showPolicyModal('privacy')" class="hover:text-white transition"><?= __("privacy_policy") ?></a>
            </div>
        </div>
    </div>
</footer>
...
<script>
    ...
    function submitNewsletter(event) {
        ...
        // Fetch to our router mapping in public/index.php
        fetch('subscribe.php', {
            method: 'POST',
            body: formData
        })
        ...
        .catch(err => {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire('Lỗi kết nối', 'Đã xảy ra sự cố khi gửi yêu cầu. Vui lòng tải lại trang (Ctrl + F5) và thử lại!', 'error');
            } else {
                alert('Lỗi kết nối! Vui lòng tải lại trang (Ctrl + F5) và thử lại.');
            }
        });
    }
            ...
            content_en: `
                <div class="space-y-4">
                    ...
                    <ul class="list-disc pl-5 space-y-2 text-slate-600">
                        ...
                        <li><b>Encrypted Payments:</b> Online transaction pipelines are protected under bank-grade secure layers via premium global SSL certificates.</li>
                    </ul>
                </div>
            `
        }
    ...
    let hasLoadedHistory = false; // Cờ đánh dấu đã tải lịch sử session chưa

    /**
     * Mở/đóng cửa sổ AI Chat
     * Lần đầu mở sẽ tự động khôi phục lịch sử chat từ Session hoặc hiện lời chào
     */
    async function toggleAIChat() {
        ...
        if (chatWindow.classList.contains('active')) {
            ...
            if (!hasLoadedHistory) {
                ...
                // Hiển thị loading tạm thời trong lúc khôi phục lịch sử
                showLoading();
                ...
                try {
                    const response = await fetch('ai_assist.php', {
                        ...
                        body: JSON.stringify({ action: 'get_history', csrf_token: csrfToken })
                    });
                    ...
                    if (data.success && data.history && data.history.length > 0) {
                        ...
                        data.history.forEach(msg => {
                            const cleanText = msg.text
                                .replace(/\n{3,}/g, '\n\n')
                                .replace(/\n/g, '<br>');
                            appendMessage(cleanText, msg.role === 'user' ? 'user' : 'ai');
                        });
                    }
                } catch (e) {
                    removeLoading();
