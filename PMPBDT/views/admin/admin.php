...
<head>
    ...
    <style>
        @media print {
            ...
            #sidebar, header, #sidebarOverlay, .no-print {
                ...
                <?php elseif ($page === 'revenue'): ?>
                    <div class="p-6">
                        ...
                        <div class="hidden print:block border-b-2 border-slate-800 pb-4 mb-6">
                            <h1 class="text-3xl font-black text-center text-slate-800 mb-2">BÁO CÁO DOANH THU & THU NHẬP</h1>
                            <div class="text-center text-sm text-slate-600 font-medium">
                                ...
                                <?php elseif ($start_date && $end_date): ?>
                                    Bộ lọc: Từ ngày <?= date('d/m/Y', strtotime($start_date)) ?> đến ngày <?= date('d/m/Y', strtotime($end_date)) ?>
                                <?php else: ?>
                                    Thời gian: Tất cả các tháng (Năm <?= date('Y') ?>)
                                <?php endif; ?>
                            </div>
                            <div class="text-center text-xs text-slate-400 mt-1">Ngày lập báo cáo: <?= date('d/m/Y H:i') ?> | Người lập: <?= htmlspecialchars($_SESSION['fullname']) ?></div>
                        </div>

                        <div class="flex justify-between items-center mb-8 no-print">
                            <h2 class="text-2xl font-black text-slate-800">Lịch sử thu nhập</h2>
                            <div class="flex gap-2">
                                <a href="export_revenue.php?<?= http_build_query(array_intersect_key($_GET, array_flip(['month', 'start_date', 'end_date']))) ?>" class="px-4 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 transition flex items-center shadow-sm">
                                    <i class="fa-solid fa-file-excel mr-2"></i> Xuất Excel
                                </a>
                                <button onclick="window.print()" class="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg font-bold hover:bg-slate-200 transition shadow-sm">
                        ...
                        <div class="bg-slate-50 border border-slate-200/60 p-5 rounded-2xl mb-8 flex flex-col gap-5 no-print">
                            <!-- Lọc nhanh theo Tháng -->
                            <div>
                                <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3"><i class="fa-solid fa-calendar-days mr-1 text-blue-500"></i> Lọc nhanh theo tháng</div>
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    ...
                                    <?php if (empty($transactions)): ?>
                                        <tr>
                                            <td colspan="5" class="py-8 text-center text-slate-400 font-medium">Không tìm thấy giao dịch nào phù hợp với bộ lọc.</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                ...
                <?php elseif ($page === 'login_history'): ?>
                    ...
                    if (!function_exists('parseBrowserInfoAdmin')) {
                        function parseBrowserInfoAdmin($ua) {
                            ...
                            if (strpos($ua, 'Edg/') !== false) { $browser = 'Microsoft Edge'; $browserIcon = 'fa-brands fa-edge'; }
