...
try {
    ...
    if (!$user) {
        ...
        $phoneBase = 'google-' . preg_replace('/[^a-zA-Z0-9]/', '', $googleId);
        if ($phoneBase === 'google-') {
            $phoneBase = 'google-' . preg_replace('/[^a-zA-Z0-9]/', '', $email);
        }
        $phone = $phoneBase;
        $j = 1;
        $checkPhone = $db->prepare('SELECT COUNT(*) FROM users WHERE phone = ?');
        while (true) {
        ...
        if (($user['auth_provider'] ?? 'local') === 'google' && empty($user['phone'])) {
            $phoneBase = 'google-' . preg_replace('/[^a-zA-Z0-9]/', '', $googleId);
            if ($phoneBase === 'google-') {
                $phoneBase = 'google-' . preg_replace('/[^a-zA-Z0-9]/', '', $email);
            }
            $phone = $phoneBase;
            $j = 1;
            $checkPhone = $db->prepare('SELECT COUNT(*) FROM users WHERE phone = ? AND id <> ?');
            while (true) {
                $checkPhone->execute([$phone, $user['id']]);
                if ((int) $checkPhone->fetchColumn() === 0) {
                    break;
                }
                $phone = $phoneBase . $j;
            ...
            $updatePhone->execute([$phone, $user['id']]);
            $user['phone'] = $phone;
        }
    }
