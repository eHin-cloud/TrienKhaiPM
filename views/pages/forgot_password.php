...
<body class="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-fuchsia-950 font-sans">
    ...
    <main class="relative z-10 flex min-h-screen items-center justify-center px-4 py-8">
        <div
            class="w-full max-w-5xl overflow-hidden rounded-[30px] bg-white shadow-[0_30px_80px_-30px_rgba(15,23,42,0.6)] ring-1 ring-white/10">
            <div class="grid min-h-[620px] grid-cols-1 lg:grid-cols-[0.95fr_1.05fr]">
                <div
                    class="relative overflow-hidden bg-gradient-to-br from-[#0f5fe6] via-[#0b4fbf] to-[#0a2f7a] p-8 text-white md:p-10">
                    ...
                    <div class="relative z-10 flex h-full flex-col justify-between">
                        <div>
                            <div
                                class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] backdrop-blur">
                ...
                <div class="bg-slate-50 p-5 sm:p-6 md:p-8 flex items-center">
                    <div class="mx-auto w-full max-w-xl">
                        <div class="mb-4">
                            <a href="/PMPBDT/public/index.php?route=index.php"
                        ...
                        <?php if ($message): ?>
                            <div
                                class="mb-4 rounded-2xl border px-4 py-3 text-sm <?= $messageType === 'success' ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700' ?>">
                                <i
                                    class="fa-solid <?= $messageType === 'success' ? 'fa-circle-check' : 'fa-circle-exclamation' ?> mr-1"></i><?= htmlspecialchars($message) ?>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-xl md:p-8">
                            <?php if ($showForm): ?>
                                ...
                                <form method="POST" class="mt-6 space-y-4">
                                    ...
                                    <button type="submit"
                                        class="w-full rounded-2xl bg-primary py-3.5 font-bold text-white shadow-lg shadow-primary/20 transition hover:bg-blue-800">Gửi
                                        mã OTP</button>
                                </form>
                                ...
                                <?php if (!empty($_SESSION['reset_password_otp'])): ?>
                                    <div class="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4">
                                        <div class="flex items-center justify-between gap-3">
                                            <div>
                                                <p class="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Thời gian
                                                    chờ gửi lại</p>
                                                <p id="resend-countdown" class="mt-1 text-2xl font-black text-slate-800">01:00
                                                </p>
                                            </div>
                                            <div class="text-right text-xs text-slate-500">
                                                <p>OTP còn hiệu lực</p>
                                                <p><?= !empty($otpExpiresAt) ? 'Hết hạn lúc ' . date('H:i', $otpExpiresAt) : '10 phút' ?>
                                                </p>
                                            </div>
                                        </div>
                                        <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-blue-100">
                                            <div id="resend-progress"
                                                class="h-full w-full rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-1000">
                                            </div>
                                        </div>
                                    </div>
                                    <form method="POST" class="mt-3">
                                        ...
                                        <button type="submit" name="resend_otp" value="1" id="resend-otp-btn"
                                            class="w-full rounded-2xl border border-slate-200 bg-white py-3.5 font-semibold text-slate-700 transition hover:bg-slate-50">Gửi
                                            lại OTP qua Gmail</button>
                            ...
                            <?php else: ?>
                                ...
                                <form method="POST" class="mt-6 space-y-4">
                                    ...
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-slate-700">Xác nhận mật khẩu
                                            mới</label>
                                        <input type="password" name="confirm_password" required minlength="8"
                                            class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800 outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10">
                                    </div>
                                    <button type="submit"
                                        class="w-full rounded-2xl bg-secondary py-3.5 font-bold text-primary shadow-lg shadow-secondary/20 transition hover:bg-yellow-400">Đặt
                                        lại mật khẩu</button>
                                </form>
                            <?php endif; ?>
                        </div>
