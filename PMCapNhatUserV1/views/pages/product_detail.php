...
// Truy xuất danh sách sản phẩm cùng nhóm (gợi ý)
$related = getRelatedProducts($db, $product['category_id'], $id, 6);

// Truy xuất danh sách sản phẩm cùng thương hiệu
...
<div class="container mx-auto px-4 py-4 md:py-6">
    ...
    <div class="text-[13px] text-primary mb-4 flex items-center gap-2 overflow-x-auto whitespace-nowrap hide-scrollbar">
        ...
        <a href="index.php?cat_id=<?= $product['category_id'] ?>"
            class="hover:underline font-medium"><?= htmlspecialchars($product['category_name']) ?></a>
        <i class="fa-solid fa-angle-right text-[10px] text-gray-400"></i>
    ...
    <div class="mb-4 pb-4 border-b border-gray-200">
        ...
        <div class="flex flex-wrap items-center gap-4 text-[13px]">
            <div class="flex items-center gap-1 text-yellow-400">
                ...
                <a href="#reviews" class="text-primary hover:underline ml-1">(<?= $product['total_reviews'] ?> đánh
                    giá)</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 flex flex-col gap-4">
            ...
            <div
                ...
                <div class="flex items-start gap-2"><i class="fa-solid fa-rotate text-primary mt-1 text-base w-5"></i>
                    <p>Hư gì đổi nấy <b>12 tháng</b> tại siêu thị toàn quốc.</p>
                </div>
                <div class="flex items-start gap-2"><i
                        class="fa-solid fa-shield-halved text-primary mt-1 text-base w-5"></i>
                    <p>Bảo hành chính hãng <b>2 năm</b>, có người đến tận nhà.</p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 flex flex-col gap-4">
            ...
            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 shadow-sm relative overflow-hidden">
                ...
                <?php if ($product['old_price']): ?>
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="text-[11px] bg-red-100 text-red-600 font-bold px-2 py-0.5 rounded border border-red-200">Giảm
                            <?= $disc ?? 0 ?>%</span>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="flex flex-col gap-3 mt-2">
                ...
                <button
                    class="w-full bg-[#2e7dd6] text-white rounded-lg py-2.5 hover:bg-[#2368b8] transition shadow-sm flex flex-col items-center justify-center"
                    onclick="document.getElementById('installmentModal').classList.remove('hidden')">
                    <span class="font-medium text-[15px] mb-0.5">Mua trả góp 0% <i
                            class="fa-solid fa-angle-right text-[12px]"></i></span>
                    <span class="text-[12px] font-normal opacity-90">Nhân viên tổng đài sẽ liên hệ lại với quý khách
                        trong 5 phút.</span>
                </button>
            </div>

            <div class="text-center text-[13px] text-gray-600 mt-2">
                Gọi đặt mua <a href="tel:18001061" class="text-primary font-bold hover:underline">1800.1061</a> (7:30 -
                22:00)
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-8">
        <div class="lg:col-span-7 bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-8">
            ...
            <button id="btn-read-more" onclick="toggleDescription()"
                class="mt-4 w-full text-primary border border-primary hover:bg-blue-50 py-2 rounded-lg text-[14px] font-medium transition">Đọc
                thêm bài viết</button>
        </div>

        <div class="lg:col-span-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-6 sticky top-20">
                <h2 class="text-[18px] font-bold text-gray-800 mb-4 pb-2 border-b border-gray-200">Thông số kỹ thuật
                </h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-8 mt-8" id="reviews">
        <h2 class="text-[18px] font-bold text-gray-800 mb-6 pb-2 border-b border-gray-200">Đánh giá & Nhận xét
                ...
                <?php for ($s = 5; $s >= 1; $s--):
                    ...
                    <div class="flex items-center gap-2 mb-1.5">
                        ...
                        <span class="text-[11px] text-gray-400 w-12"><?= $count ?> <span
                                class="hidden sm:inline">lượt</span></span>
                    </div>
                <?php endfor; ?>
            </div>
                ...
                <form method="POST" action="product_detail.php?id=<?= $id ?>" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i class="fa-solid fa-camera mr-1"></i>
                            Thêm ảnh / video (tối đa 5 file, mỗi file ≤ 20MB)</label>
                        <div id="media-drop-zone"
                            ...
                            <div id="media-placeholder">
                                ...
                                <p class="text-sm text-gray-500">Kéo thả hoặc <span class="text-primary font-medium">bấm để
                                        chọn</span> ảnh/video</p>
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="toggleReviewForm()"
                            class="px-4 py-2 text-gray-500 hover:bg-gray-200 rounded-lg text-sm font-medium transition">Hủy</button>
            ...
            <?php else: ?>
                <div class="text-center py-4">
                    ...
                    <button onclick="document.getElementById('loginModal').classList.remove('hidden')"
                        class="bg-primary text-white py-2 px-6 rounded-lg text-sm font-bold shadow hover:bg-blue-800 transition">Đăng
                        nhập ngay</button>
                </div>
            <?php endif; ?>
        </div>
    ...
    <!-- =========================================================
         SẢN PHẨM LIÊN QUAN (Sản phẩm cùng danh mục)
         ========================================================= -->
    <?php if (!empty($related)): ?>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-8 mt-8">
            <h2 class="text-lg font-bold text-gray-800 mb-4 uppercase border-b border-gray-100 pb-2">Sản phẩm cùng danh mục
            </h2>
    ...
    <?php if (!empty($same_brand_products)): ?>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-8 mt-8">
            <h2 class="text-lg font-bold text-gray-800 mb-4 uppercase border-b border-gray-100 pb-2">Sản phẩm cùng hãng
                <?= htmlspecialchars($product['brand_name']) ?></h2>
            <div class="flex overflow-x-auto gap-3 pb-4 hide-scrollbar">
                <?php foreach ($same_brand_products as $r): ?>
                    <a href="product_detail.php?id=<?= $r['id'] ?>"
                        class="min-w-[160px] w-[160px] flex-shrink-0 border border-gray-100 hover:border-primary p-3 rounded-lg group transition block shadow-sm hover:shadow-md">
                        <div class="h-32 flex items-center justify-center mb-3">
                            <img src="<?= $r['image'] ?>"
                                class="max-w-full max-h-full object-contain group-hover:scale-105 transition">
                        </div>
...
<div id="installmentModal"
    ...
    <div class="bg-white rounded-xl w-full max-w-[400px] flex flex-col relative shadow-2xl">
        ...
        <form id="installmentForm" class="p-5" onsubmit="submitInstallment(event)">
            ...
            <button type="submit"
                class="w-full bg-primary text-white font-bold py-2.5 rounded-lg hover:bg-blue-800 transition shadow">Xác
                nhận Đăng ký</button>
            <p class="text-center text-[11px] text-gray-500 mt-3">Nhân viên tổng đài sẽ liên hệ lại với quý khách trong
                5 phút.</p>
        </form>
    </div>
</div>
