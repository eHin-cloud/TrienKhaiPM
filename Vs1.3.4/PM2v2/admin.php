...
// ==========================================
// XỬ LÝ CÁC ACTION (THÊM, SỬA, XÓA, DUYỆT)
// ==========================================
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_order_status') {
        ...
        $msg_type = 'success';
    } elseif ($action === 'delete_order' && $user_role === 'admin') {
        $id = $_POST['id'];
        $db->prepare("DELETE FROM order_details WHERE order_id=?")->execute([$id]);
...
if ($page === 'orders') {
    ...
    if ($search) $sql .= " WHERE fullname LIKE '%$search%' OR phone LIKE '%$search%' OR id = '$search'";
    $sql .= " ORDER BY id DESC";
    $items = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
    
    // Nạp chi tiết các sản phẩm trong từng đơn hàng
    $all_details = $db->query("SELECT od.*, p.name, p.image FROM order_details od JOIN products p ON od.product_id = p.id")->fetchAll(PDO::FETCH_ASSOC);
    $details_by_order = [];
    foreach ($all_details as $d) {
        $details_by_order[$d['order_id']][] = $d;
    }
    // Ghép mảng chi tiết vào mảng đơn hàng
...
<body class="bg-gray-100 font-sans flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 text-white flex flex-col h-full shadow-2xl z-20 shrink-0">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <a href="?p=orders" class="flex items-center gap-3 px-4 py-3 rounded-lg transition <?= $page==='orders' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800' ?>">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 z-10 shrink-0">
            <h2 class="text-xl font-bold text-gray-800">
                <?php
                    if($page==='orders') echo "Quản lý Đơn Hàng";
                    elseif($page==='products') echo "Quản lý Sản Phẩm";
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <?php if ($page === 'orders'): ?>
                    <div class="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                        <span class="text-gray-600 font-medium">Tổng cộng: <?= count($items) ?> đơn hàng</span>
                    </div>
                    <table class="w-full text-left border-collapse text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-50 border-b border-gray-100 transition">
                                ...
                                <td class="p-4">
                                    <div class="font-medium text-gray-800"><?= htmlspecialchars($item['fullname']) ?> - <?= htmlspecialchars($item['phone']) ?></div>
                                    <div class="text-gray-500 text-xs truncate max-w-xs"><?= htmlspecialchars($item['address']) ?></div>
                                    <?php if($item['note']): ?>
                                        <div class="text-xs mt-1 <?= strpos($item['note'], 'chuyển khoản') !== false ? 'text-blue-600 font-bold' : 'text-gray-400' ?>"><i class="fa-solid fa-comment-dots"></i> <?= htmlspecialchars($item['note']) ?></div>
                                    <?php endif; ?>
                                </td>
                                <td class="p-4 font-bold text-danger"><?= number_format($item['total_price']) ?>đ</td>
                                <td class="p-4">
                                    <?php 
                                        if($item['status'] == 'pending') echo '<span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs font-bold border border-yellow-200">Chờ xử lý COD</span>';
                                        elseif($item['status'] == 'processing') echo '<span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-200 animate-pulse block w-fit">Báo đã CK - Chờ duyệt</span>';
                                        elseif($item['status'] == 'delivering') echo '<span class="bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-xs font-bold border border-indigo-200">Đang giao hàng</span>';
                                        elseif($item['status'] == 'completed') echo '<span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold border border-green-200">Hoàn thành</span>';
                                        elseif($item['status'] == 'cancelled') echo '<span class="bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold border border-red-200">Đã hủy</span>';
                                    ?>
                                </td>
                                <td class="p-4 text-gray-500 text-xs"><?= date('d/m/Y H:i', strtotime($item['created_at'])) ?></td>
                                <td class="p-4 text-center">
                                    <button onclick='viewOrder(<?= json_encode($item) ?>)' class="text-blue-500 hover:bg-blue-50 w-8 h-8 rounded-full transition" title="Xem chi tiết & Cập nhật"><i class="fa-solid fa-eye"></i></button>
                                    <?php if ($user_role === 'admin'): ?>
                                    <form method="POST" class="inline-block" onsubmit="return confirmDelete(event)">
                                        <input type="hidden" name="action" value="delete_order">
                                        <input type="hidden" name="id" value="<?= $item['id'] ?>">
                                        <button type="submit" class="text-red-500 hover:bg-red-50 w-8 h-8 rounded-full transition" title="Xóa đơn hàng"><i class="fa-solid fa-trash"></i></button>
                                    </form>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if(empty($items)): ?><tr><td colspan="6" class="p-8 text-center text-gray-500">Không có đơn hàng nào</td></tr><?php endif; ?>
                        </tbody>
                    </table>
    ...
    <div id="orderModal" class="hidden fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative">
            <div class="sticky top-0 bg-white px-6 py-4 border-b border-gray-200 flex justify-between items-center z-10">
                ...
                <button type="button" onclick="closeModal('orderModal')" class="text-gray-400 hover:text-red-500 text-xl"><i class="fa-solid fa-xmark"></i></button>
            </div>
            
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Thông tin khách -->
                <div>
                    <h4 class="font-bold text-gray-700 mb-3 border-b pb-2"><i class="fa-solid fa-user-circle mr-1 text-primary"></i> Thông tin khách hàng</h4>
                    <div class="space-y-2 text-sm">
                        <p><span class="text-gray-500 w-20 inline-block">Họ tên:</span> <b id="detail_fullname" class="text-gray-800"></b></p>
                        <p><span class="text-gray-500 w-20 inline-block">Điện thoại:</span> <b id="detail_phone" class="text-gray-800"></b></p>
                        <p class="flex items-start"><span class="text-gray-500 w-20 inline-block shrink-0">Địa chỉ:</span> <span id="detail_address" class="text-gray-800"></span></p>
                        <p class="flex items-start"><span class="text-gray-500 w-20 inline-block shrink-0">Ghi chú:</span> <span id="detail_note" class="text-red-600 font-medium"></span></p>
                    </div>
                </div>

                <!-- Cập nhật trạng thái -->
                <div class="bg-blue-50 p-5 rounded-xl border border-blue-100 shadow-sm">
                    <h4 class="font-bold text-blue-800 mb-3"><i class="fa-solid fa-truck-fast mr-1"></i> Cập nhật trạng thái Đơn hàng</h4>
                    <form method="POST">
                        ...
                        <select name="status" id="detail_status" class="w-full px-4 py-2.5 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none mb-4 font-bold text-gray-700 bg-white shadow-sm">
                        ...
                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-lg transition shadow-md flex justify-center items-center gap-2">
                ...
                <div class="md:col-span-2 mt-2">
                    <h4 class="font-bold text-gray-700 mb-3 border-b pb-2"><i class="fa-solid fa-box-open mr-1 text-primary"></i> Danh sách sản phẩm đã mua</h4>
                    <div class="overflow-x-auto border border-gray-200 rounded-lg">
                        <table class="w-full text-left text-sm border-collapse">
                            ...
                            <tfoot class="bg-gray-50">
                                <tr>
                                    <td colspan="3" class="p-3 text-right font-bold text-gray-700 uppercase text-xs">Tổng tiền phải thu:</td>
                                    <td class="p-3 text-right font-extrabold text-danger text-lg" id="detail_total_price"></td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            <form method="POST" class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="usr_password" minlength="8" class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 outline-none transition">
                        <div id="admin-pw-hints" class="mt-1.5 text-xs space-y-0.5">
                            <p id="admin-pw-len" class="flex items-center gap-1 text-gray-400"><i class="fa-solid fa-circle text-[6px]"></i> Ít nhất 8 ký tự</p>
                            <p id="admin-pw-letter" class="flex items-center gap-1 text-gray-400"><i class="fa-solid fa-circle text-[6px]"></i> Ít nhất 1 chữ cái</p>
                        </div>
