...
$orderId = (int)($payload['data']['orderCode'] ?? $payload['orderCode'] ?? 0);
...
if (in_array($status, ['paid', 'success', 'completed'], true)) {
    $stmt = $db->prepare("UPDATE orders SET status = 'processing', note = CONCAT(IFNULL(note, ''), ' [PayOS Webhook] Thanh toán xác nhận') WHERE id = ?");
