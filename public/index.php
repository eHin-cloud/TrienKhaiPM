...
if (!empty($_SESSION['user_id'])) {
    try {
        ...
        if ($userStatus && (int)($userStatus['is_banned'] ?? 0) === 1) {
            ...
            // Chuyển hướng về trang chủ kèm tham số thông báo khóa tài khoản
            header('Location: index.php?banned=1');
            exit;
        }
    } catch (\Exception $e) {
        // Bỏ qua lỗi CSDL để tránh gián đoạn hệ thống
    }
}
...
// Xử lý trường hợp đặc biệt: Nếu người dùng truy cập root, thư mục public hoặc thư mục con chứa dự án ở local,
// ta mặc định coi đó là trang chủ (index.php).
$projectDir = basename(dirname(__DIR__));
if ($route === '' || $route === 'public' || $route === 'index.php' || $route === $projectDir) {
    $route = 'index.php';
}
...
$routesMap = [
    ...
    'order_details.php'  => '../views/api/order_details.php', // Tải chi tiết đơn hàng AJAX
    'subscribe.php'      => '../views/api/subscribe.php', // Đăng ký nhận ưu đãi
...
// 3c. Tự động định tuyến sang API JSON của mobile app nếu có Header Accept: application/json hoặc Authorization
$mobileApiFallbackMap = [
    'auth.php'           => 'auth.php',
    ...
    'installment.php'    => 'installment.php',
    'review.php'         => 'review.php',
    'notification.php'   => 'notification.php',
    'voucher.php'        => 'voucher.php',
];

$acceptHeader = $_SERVER['HTTP_ACCEPT'] ?? '';
$authHeader = $_SERVER['HTTP_AUTHORIZATION'] ?? $_SERVER['REDIRECT_HTTP_AUTHORIZATION'] ?? '';
$isJsonOrMobile = (stripos($acceptHeader, 'application/json') !== false) || !empty($authHeader);

if ($isJsonOrMobile && array_key_exists($route, $mobileApiFallbackMap)) {
    require_once __DIR__ . '/api/' . $mobileApiFallbackMap[$route];
    exit;
}
